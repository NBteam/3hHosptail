...
//        retSize = [self.model.content sizeWithFont:[[self class] textLabelFont] maxSize:textBlockMinSize];
...
- (void)setModel:(MessageModel *)model
{
    ...
    //如果发送的是病例，需要添加点击事件
    if ([self.model.message.ext objectForKey:@"illCaseId"]) {
        NSMutableAttributedString *protocolDescript = [[NSMutableAttributedString alloc] initWithString:self.model.content];
        NSRange protocolStrRange = {0,[protocolDescript length]};
        [protocolDescript addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithHEX:0x397acc] range:protocolStrRange];
        [protocolDescript addAttribute:NSUnderlineStyleAttributeName value:[NSNumber numberWithInteger:NSUnderlineStyleSingle] range:protocolStrRange];
        _textLabel.attributedText = protocolDescript;
        _textLabel.userInteractionEnabled = YES;
        UIButton *btn = [CreatUIUnit creatButtonWithTitile:nil backGroundImage:nil fontSize:0 titleColor:nil parentView:_textLabel];
        btn.frame = _textLabel.bounds;
...
- (void)btnAction
{
    _labelBlock([self.model.message.ext objectForKey:@"illCaseId"],[self.model.message.ext objectForKey:@"userId"]);
}
