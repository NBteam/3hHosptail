...
#pragma mark push到简历页面

//- (void)pushCaseManagementVc{
//    CaseManagementViewController *caseManagementVc = [[CaseManagementViewController alloc] init];
//    caseManagementVc.indexString = @"chatVc";
//    caseManagementVc.isFromIM = YES;
//    [caseManagementVc setCaseManagementModelBlock:^(CaseManagementModel *model) {
//        KYUser *user = [KYUser ReadLocalUserDataForPath:[UserPath stringByAppendingPathComponent:@"User"]];
//      //得到病例model
//        NSDictionary *ext = @{@"illCaseId":@(model.id),@"illCaseName":model.name,@"userId":@(user.user_id),@"userName":user.true_name};
//        [self sendTextMessage:model.name ext:ext];
//    }];
//    [self.navigationController pushViewController:caseManagementVc animated:YES];
//}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            //设置头像
            model.myHeadImage = [NSURL URLWithString:self.myHeadImage];
            model.yourHeadImage = [NSURL URLWithString:self.yourHeadImage];
            ...
            if (cell == nil) {
                ...
                cell.chatTextBlock = ^(NSString *ext,NSString *userId){
                    NSLog(@"ext is %@",ext);
//                    CaseDetailViewController *patientCaseDetailVC = [CaseDetailViewController new];
//                    patientCaseDetailVC.caseId = [ext integerValue];
//                    patientCaseDetailVC.isFromIM = YES;
////                    patientCaseDetailVC.userId = [userId integerValue];
//                    KYBaseNavigationViewController *nav = [[KYBaseNavigationViewController alloc] initWithRootViewController:patientCaseDetailVC];
