...
@interface MyAppointmentTopView()
@property (nonatomic, strong) NSArray *titleArray;

@end
@implementation MyAppointmentTopView
...
- (void)customButtons{
    for (int i = 0; i < self.titleArray.count; i++) {
        ...
        [self addSubview:btn];
        if (i>0) {
            UILabel *lab = [[UILabel alloc] initWithFrame:CGRectMake((DeviceSize.width/3 -0.25) *i, 10, 0.5, self.height - self.labLine.height -20)];
