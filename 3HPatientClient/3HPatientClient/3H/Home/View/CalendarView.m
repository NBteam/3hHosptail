...
- (CGFloat)reloadCalendarView:(NSMutableArray *)array{
    ...
    return [self calendarSetDate:changeDate];
}
...
- (void)btnUpAction{
    ...
    ScheduleCalendarModel *model = self.dataArray[0];
    
    
    if (self.calendarBtnBlock) {
        self.calendarBtnBlock([NSString stringWithFormat:@"%@",model.pre_date_m]);
    }
//    [self reloadTitle:changeYear month:changeMonth day:changeDay index:0];
...
- (void)btnDownAction{
    ...
    if (self.calendarBtnBlock) {
        self.calendarBtnBlock([NSString stringWithFormat:@"%@",model.next_date_m]);
    }
...
-(CGFloat)calendarSetDate:(NSDate *)date
{
...
-(CGFloat)drawBtn
{
    ...
    for (int i = firstDayIndexOfWeek - 1 ; i < monthRange.length + firstDayIndexOfWeek -1 ; i ++)
    {
      ...
      forControlEvents:UIControlEventTouchUpInside];
        ...
        NSString *dayString;
        if (i + 2 - firstDayIndexOfWeek<10) {
            dayString= [NSString stringWithFormat:@"0%i",i + 2 - firstDayIndexOfWeek];
        }else{
           dayString= [NSString stringWithFormat:@"%i",i + 2 - firstDayIndexOfWeek];
        }
        
        NSString *month;
        
        if (changeMonth<10) {
            month = [NSString stringWithFormat:@"0%li",changeMonth];
        }else{
            month = [NSString stringWithFormat:@"%li",changeMonth];
        }
        ScheduleCalendarModel *model = self.dataArray[0];

        for (NSDictionary *dict in model.list) {

            if ([dict[@"date"] isEqualToString:[NSString stringWithFormat:@"%li-%@-%@",changeYear,month,dayString]]) {
                if ([dict[@"drug_tip_num"] integerValue] !=0||[dict[@"recheck_tip_num"] integerValue] !=0||[dict[@"tip_num"] integerValue] !=0) {
